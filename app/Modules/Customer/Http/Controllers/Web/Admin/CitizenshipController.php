...
class CitizenshipController extends BaseController
{
     ...
     * @var string
     */
    public $moduleTitle = 'citizenship';

    /**
     * @var string
     */
    public $viewFolderName = 'citizenship';
    ...
    public function __construct()
    {
        ...
        $this->baseData['moduleKey'] = 'citizenship';
        $this->baseData['baseRouteName'] = $this->baseData['baseRouteName'] . '.' . $this->baseData['moduleKey'] . '.';
        $this->baseData['trans_text'] = CitizenshipHelper::getLang();
    }
    ...
    {
        $this->baseData['allData'] = Citizenship::orderBy('created_at','desc')->paginate(25);
    ...
    {
        try {
            $this->baseData['routes']['create_form_data'] = CitizenshipHelper::getRoutes()['create_data'];
    ...
    {
        try {
            $this->baseData['routes'] = CitizenshipHelper::getRoutes();
            $this->baseData['routes']['upload_image'] = route(config('cms.admin.base_route_name') .'.image.upload');

            if ($request->get('id')) {
                $item = Citizenship::findOrFail($request->get('id'))->load(['translations']);
                $locales = TranslationFieldsWithLocale::setTranslations($item->translations)->setTranslationFields((new CitizenshipTranslation())->getFillable())
    ...
    {
        try {
            $saveData = Arr::except($request->except(['id'])['main'], array_merge(['profile'],(new CitizenshipTranslation())->getFillable()));
            if ($request->get('id')) {
                $item = Citizenship::findOrFail($request->get('id'));
                $item->update($saveData);
            } else {
                $item = Citizenship::create($saveData);
            }
    ...
    {
        try {
            Citizenship::findOrFail($request->get('id'))->delete();
        } catch (\Exception $ex) {
            return ServiceResponse::jsonNotification($ex->getMessage(), $ex->getCode(), []);
        }
    ...
    {
        try {
            Citizenship::findOrFail($request->get('id'))->update(['status' => $request->get('status')]);
        } catch (\Exception $ex) {
            return ServiceResponse::jsonNotification($ex->getMessage(), $ex->getCode(), []);
        }
