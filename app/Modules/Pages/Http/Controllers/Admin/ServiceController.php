<?php
/**
 *  app\Modules\Pages\Http\Controllers\Admin\ServiceController.php
 *
 * Date-Time: 9/21/2021
 * Time: 5:26 AM
...
use App\Modules\Pages\Helpers\ServiceHelper;
use App\Modules\Pages\Http\Requests\Admin\Service\ServiceStoreRequest;
use App\Modules\Pages\Models\Service;
use App\Modules\Pages\Models\Translations\ServiceTranslation;
use App\Modules\Pages\Services\Admin\ServiceStoreData;
...
class ServiceController extends BaseController
{
     ...
     * @var string
     */
    public $moduleTitle = 'service';

    /**
     * @var string
     */
    public $viewFolderName = 'service';
    ...
    public function __construct()
    {
        ...
        $this->baseData['moduleKey'] = 'service';
        $this->baseData['baseRouteName'] = $this->baseData['baseRouteName'] . '.' . $this->baseData['moduleKey'] . '.';
        $this->baseData['trans_text'] = ServiceHelper::getLang();
    }
    ...
    {
        $this->baseData['allData'] = Service::orderBy('created_at', 'desc')->paginate(25);
    ...
    {
        try {
            $this->baseData['routes']['create_form_data'] = ServiceHelper::getRoutes()['create_data'];
    ...
    {
        try {
            $this->baseData['routes'] = ServiceHelper::getRoutes();
            ...
            if ($request->get('id')) {
                $item = Service::findOrFail($request->get('id'))->load(['translations']);
                $locales = TranslationFieldsWithLocale::setTranslations($item->translations)->setTranslationFields((new ServiceTranslation())->getFillable())
    ...
    {
        try {
            Service::findOrFail($request->get('id'))->update(['status' => $request->get('status')]);
        } catch (\Exception $ex) {
            return ServiceResponse::jsonNotification($ex->getMessage(), $ex->getCode(), []);
        }
