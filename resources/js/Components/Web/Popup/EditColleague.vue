<template>
    <popup
        v-if="visible"
        :title="__('თანამშრომლის რედაქტირება')"
    >
...
export default {
    ...
    mounted() {
        emitter.on('showEditColleaguePopup', (item) => {
            this.form.id = item.employee.id;
            this.form.company_id = item.employee.company_id;
            this.form.name = item.employee.name;
            this.form.email = item.employee.email;
            this.form.phone = item.employee.phone;
            this.form.role = item.employee.role;

            this.companies = item.companies.map((el) => (
                {
                    label: el.title,
                    value: el.id
                }
            ))
            const defaultCompany = item.companies.filter(e => e.id).find(el => el.id === item.employee.company_id)
            this.defaultCompany = {
                label: defaultCompany.title,
